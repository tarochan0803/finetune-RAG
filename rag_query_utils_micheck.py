# rag_query_utils_micheck.py (AttributeError修正版)
# -*- coding: utf-8 -*-
...
# --- 設定ファイルと関連ライブラリのインポート ---
try:
    # micheck版のconfigをインポート
...
try:
    ...
    from langchain_core.documents import Document
     ...
     sys.exit(f"LangChain import error in rag_query_utils_micheck.py: {e}. Please install required libraries.")

try:
    # utils_micheck から必要な関数をインポート
    from utils_micheck import format_document_snippet
except ImportError as e:
    sys.exit(f"Fatal Error: utils_micheck.py not found. {e}")
...
def initialize_embedding(config: Config, logger_instance: logging.Logger) -> Optional[HuggingFaceEmbeddings]:
    ...
    if not logger_instance:
        logger_instance = logging.getLogger("RAGapp_micheck_Init_Embed")
...
def load_external_chroma_db(config: Config, embedding_function: HuggingFaceEmbeddings, logger_instance: logging.Logger) -> Optional[Chroma]:
    ...
    if config.use_memory_db:
        logger_instance.critical("Configuration error: 'use_memory_db' is True, cannot load external DB.")
        return None
...
def generate_answer_with_gemini(question: str, context: str, config: Config, logger_instance: logging.Logger) -> str:
    ...
    if not GEMINI_AVAILABLE or not genai:
        logger_instance.error("Gemini library is not available."); return "[Error: Gemini library not found]"
    if not config.gemini_api_key:
        logger_instance.error("Gemini API key is not configured."); return "[Error: Gemini API key not set]"
    ...
    try:
        ...
        gen_config = genai.types.GenerationConfig(
            temperature=config.synthesizer_temperature, top_p=config.synthesizer_top_p, max_output_tokens=config.synthesizer_max_new_tokens
        )
        ...
        time.sleep(config.api_call_delay_seconds) # API待機
        ...
        elif "ResourceExhausted" in str(e) or ("429" in str(e)): return "[Error: Gemini API quota exceeded or rate limit hit]"
        # 他のエラーも捕捉するならここに追加
...
def process_single_variant(
    query: str, config: Config, vectordb: Chroma, embedding_function: HuggingFaceEmbeddings,
    logger_instance: logging.Logger, metadata_filter: Optional[dict] = None, k_value: int = 5
    ...
    try:
        ...
        else: context_snippet = format_document_snippet(context, 150) # スニペット長は調整可
        ...
        answer = generate_answer_with_gemini(query, context, config, logger_instance) # 内部で待機
...
def ask_question_single_variant(
    ...
    try:
        ...
        # process_single_variant を呼び出し
        answer, _, retrieved_docs = process_single_variant(
            query=query, config=config, vectordb=vectordb, embedding_function=embedding_function,
            logger_instance=logger_instance, # ★ 正しいロガーインスタンスを渡す
            metadata_filter=metadata_filter, k_value=config.rag_k # configからk値を取得
        )
        final_answer_text = answer
        source_documents = retrieved_docs # 検索に使用されたソースドキュメントを結果に含める
        # ★★★ 修正済み: logger.instance -> logger_instance ★★★
        logger_instance.info("Single variant processing finished successfully.")
        ...
        source_documents = [] # エラー時はソースは空

    # 最終回答をイテレータに変換 (Streamlitの write_stream を使う場合は必要だが、今回は使っていない)
    # 単一の文字列を返す形式にするか、現在の辞書形式を維持するか検討
    # RAGapp側で join しているので、イテレータでなくても良いが、互換性のため残す
    final_answer_stream = iter([final_answer_text])

    time_end_total = time.time()
    logger_instance.info(f"Total query processing time for '{query[:30]}...': {time_end_total - start_time_total:.2f} sec.")
    # 戻り値の辞書構造は維持
    return {
        "result_stream": final_answer_stream, # RAGapp側で join する想定
        "source_documents": source_documents, # 検索に使われたソース
        "variant_answers": [final_answer_text] # 今回はVariantは1つだが形式を合わせる
    }